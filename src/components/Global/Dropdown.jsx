    ...
    <Menu as="div" className="relative">
      <Menu.Button className="w-full flex items-center justify-center text-white">
        {trigger ? (
          ...
          <button
            variant="ghost"
            className="w-full flex justify-between items-center px-4 py-3 rounded-md transition-all duration-200 font-medium bg-zinc-500/5 hover:bg-zinc-500/10"
          >
            {label}
...
export function Text({ children }) {
  return (
    <div className="text-xs font-extrabold uppercase text-slate-400 dark:text-gray-600 p-2">
      {children}
    </div>
  );
}
      ...
      {({ active }) => (
        <button
          className={classNames(
            `w-full flex items-center p-2 py-2.5 text-sm rounded-md ${
              active ? "bg-zinc-500/5" : ""
