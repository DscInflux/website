...
function Input(props) {
  ...
  return (
    <div
      ...
      } rounded-lg w-full bg-light dark:bg-dark peer transition-all ease-in-out duration-200 placeholder-white/0 outline-none border border-primary/5 hover:border-primary/20 ${
        focused && "!border-primary/100"
      } ${className}`}
    >
      {leftContent && (
        <div
      ...
      <Component
        ...
        className={`${type === "password" && "pr-14"} py-2 ${
          leftContent ? "pl-1.5" : "pl-5"
        } ${
          rightContent ? "pr-1.5" : "pr-5"
        } w-full h-full !outline-none focus:ring-0 bg-transparent !border-none focus:outline-none focus:border-none ${wrapperClass}`}
        rows={rows || 4}
          ...
          return (
            ...
            >
              <div className="-ml-2 mr-3 w-10 h-10 group-hover:bg-primary/10 transition-all duration-200 rounded-lg flex items-center justify-center">
                <div
                  className={`relative w-6 h-6 border-2 ${
                    checked ? "border-primary" : "border-gray-600"
                  } group-hover:border-primary transition-all duration-200 rounded-lg flex flex-shrink-0 justify-center items-center`}
                >
                  <div
                    className={`w-6 h-6 flex items-center justify-center rounded-lg transition-all duration-200 ease-in-out bg-primary ${
              ...
              <p
                className={`font-medium ${
                  checked ? "text-black dark:text-white" : "text-gray-600"
                }`}
              >
                {item.label}
              </p>
...
export const RadioGroup = memo(function Radio(props) {
  ...
  return (
    <HRadioGroup onChange={onChange} value={value}>
      <div className="space-y-2">
        {items.map((_, i) => (
          <HRadioGroup.Option
            key={i}
            value={i}
            className={
              "relative flex cursor-pointer rounded-lg focus:outline-none group"
            }
          >
            {({ active, checked }) => (
              <>
                    ...
                    <div className="-ml-2 mr-3 w-10 h-10 group-hover:bg-primary/10 transition-all duration-200 rounded-full flex items-center justify-center">
                      <div
                        className={`w-6 h-6 border-2 ${
                          checked ? "border-primary" : "border-gray-600"
                        } group-hover:border-primary transition-all duration-200 rounded-full flex flex-shrink-0 justify-center items-center`}
                      >
                        <div
    ...
    <>
      <div className="relative w-full" id="form">
        <div className="relative rounded-md shadow-sm" ref={ref}>
          <div
            className="cursor-pointer group relative flex "
          ...
          >
            <Component
              ...
              autoComplete="off"
              name={randomId}
              className={`cursor-pointer rounded-lg w-full bg-light dark:bg-dark peer ${
                placeholder ? "!py-7 !pb-2" : "!py-4"
              } pl-[1.1rem] transition-all ease-in-out duration-200 placeholder-white/0 outline-none border border-primary/5 group-hover:border-primary/20 focus:border-primary/100`}
              placeholder=" "
              rows={rows || 4}
              required={required}
              value={selected}
        ...
        <div className="relative mt-1 rounded-md overflow-hidden shadow-sm">
          <div className="relative flex group">
            <label
              htmlFor={mathId}
              className={`${
                type === "banner" ? "w-full h-80" : "w-36 h-36"
              } relative overflow-hidden flex flex-col justify-center items-center bg-light dark:bg-dark rounded-lg border-2 border-primary/5 border-dashed cursor-pointer hover:bg-primary/5 hover:dark:bg-primary/5 transition-all duration-200`}
            >
