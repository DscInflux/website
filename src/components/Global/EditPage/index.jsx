...
export default function EditPage({
  ...
  let [isPrivateEmail, setIsPrivateEmail] = useState(
    user?.entity?.privacy?.isEmailPrivate ?? true,
  );
  let [isPrivateBirthday, setIsPrivateBirthday] = useState(
    user?.entity?.privacy?.isBirthdayPrivate ?? true,
  );
  let [isPrivateLocation, setIsPrivateLocation] = useState(
    user?.entity?.privacy?.isLocationPrivate ?? true,
  );
  let [isPrivateGender, setIsPrivateGender] = useState(
    user?.entity?.privacy?.isGenderPrivate ?? true,
  );
              ...
              {isSubmit ? (
                <>
                  ...
                  <div className="flex items-center justify-center mt-2">
                    <p className="text-dark/50 dark:text-white/50 font-regular">
                      Start introducing yourself with submitting this form.
                    </p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="flex items-center justify-center mt-4">
                    <p className="text-gray-500 dark:text-gray-400">
                      Arrange to introduce yourself to the community
                    </p>
                  </div>
                </>
              )}
              ...
              {tabs.map((tab) => (
                <MaterialButton
                  ...
                  className={classNames(
                    ...
                    activeTab === tab.id
                      ? "text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-500 duration-500"
          ...
          <div className="bg-light dark:bg-dark rounded-b-lg p-4">
            ...
            <div className="flex items-center justify-end">
              <Button
                ...
                className="font-bold text-sm flex items-center justify-center gap-2 px-4 py-4 rounded-t-lg cursor-pointer transition-all duration-200 normal-case text-primary"
              >
                <i className="fas fa-save" />
                <span>
                  {isSubmit
                    ? "Submit your profile (by clicking this you're also assenting to our TOS)"
                    : "Save changes"}
                </span>
              </Button>
            </div>
