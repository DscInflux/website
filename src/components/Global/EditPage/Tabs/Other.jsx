...
import Input, { MaterialSelect } from "../../Input";

export default function Other() {
  const { data: fromRoles } = useSWR("/v1/roles");
  const { data: fromSocials } = useSWR("/v1/socials");
      ...
      <div className="grid grid-cols-1 gap-4 w-full">
        <div>
          ...
          <Input
            ...
            placeholder="Your preferred language, e.g. English, Swedish, Hindi, etc."
          />
        </div>
        <form
          onSubmit={(e) => {
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            const social = fromSocials?.data.find(
              (item) => item.name === preSocial,
            );
            if (social) {
              const url = social?.url;
              const value = socialValue?.includes(
                url?.replace(/{username}/g, ""),
              )
                ? socialValue
                : url?.replace(/{username}/g, socialValue);
              ...
              if (socials?.find((item) => item.name === preSocial)) {
                ...
                setSocials(
                  socials?.map((item) =>
                    item.name === preSocial ? newObj : item,
                  ),
                );
              } else {
                newObj = {
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-2">
            ...
            <Button
              variant="ghost"
              type="submit"
              className="!h-[3.74rem] w-full lg:w-10 flex items-center justify-center"
            >
          ...
          <div className="mt-6 lg:mt-2 grid grid-cols-1 lg:grid-cols-3 gap-2">
            {socials?.map((item, i) => (
              <div
                key={i}
                className="animate-fade-in-down relative bg-light dark:bg-dark border border-primary/5 hover:border-primary/20 rounded-lg p-4 transition-all duration-200"
              >
                <div className="w-full flex items-center justify-between">
                  <h1 className="capitalize text-md mb-1">
                    <i
                      className={
                        item?.icon?.value + " " + item?.icon?.label + " mr-1"
                      }
                    />{" "}
                    {item?.name}{" "}
                  </h1>
                  <div
                    ...
                    onClick={() =>
                      setSocials(socials.filter((_, index) => index !== i))
                    }
                  >
                    <i className="fal fa-times" />
                  </div>
