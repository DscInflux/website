...
export default function Home({ sortings, roles, skills, languages }) {
  ...
  useEffect(() => {
    ...
    if (router.query.language) {
      const findIndex = languages.findIndex(
        (item) => item === router.query.language,
      );
      if (findIndex !== -1) {
      ...
      <div className="max-w-7xl w-full py-24">
        <div className="lg:grid grid-cols-1 lg:grid-cols-12 gap-6 px-10 2xl:px-0">
          <div className="col-span-3 2xl:col-span-2">
                        ...
                        content: (
                          <RadioGroup
                            ...
                            onChange={(e) => {
                              router.push(
                                addQuery(router, {
                                  sort: e.label.replace(" ", "-").toLowerCase(),
                                }),
                              );
                              setActive(e.value);
                            }}
            ...
            <div className="hidden lg:flex flex-col gap-4 mt-4 space-y-6">
              <div className="flex flex-col gap-2">
                  ...
                  label={
                    <>
                      <span className="text-sm text-black dark:text-white flex items-center gap-2">
                        <i
                          className={`${sortings?.[active]?.icon?.value} ${sortings?.[active]?.icon?.label} mr-2`}
                        />
                ...
                >
                  {sortings
                    ?.filter((e) => sortings?.[active]?.label !== e.label)
                    .map((item, i) => (
                      <Item
                        key={i}
                        onClick={() => {
                          router.push(
                            addQuery(router, {
                              sort: item.label.replace(" ", "-").toLowerCase(),
                            }),
                          );
                          setActive(i);
                        }}
                      ...
                      >
                        <i
                          className={`${item?.icon?.value} ${item?.icon?.label} mr-2`}
                        />
                        {item.label}
                      </Item>
                    ))}
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {languages
                    .filter((el, index) => index !== language)
                    .map((item, i) => (
                      <Item
                        key={i}
                        onClick={() => {
              ...
              <div className="flex flex-col gap-2">
                ...
                <Dropinput
                  ...
                  icon="fal fa-search"
                  onSelect={(e) =>
                    router.push(appendQuery(router, "roles", e.label))
                  }
                  items={roles
                    .filter(
                      (e) => !router.query?.roles?.split(",").includes(e.name),
                    )
                    .map((item) => ({
                      label: item.name,
                      value: item.slug,
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {router.query.roles?.split(",").map((item, i) => (
                      <div key={i} className="flex items-center gap-0.5">
                        ...
                        <div
                          onClick={() =>
                            router.push(removeQueryValue(router, "roles", item))
                          }
              ...
              <div className="flex flex-col gap-2">
                ...
                <Dropinput
                  ...
                  icon="fal fa-search"
                  onSelect={(e) =>
                    router.push(appendQuery(router, "skills", e.label))
                  }
                  items={skills
                    .filter(
                      (e) => !router.query?.skills?.split(",").includes(e.name),
                    )
                    .map((item) => ({
                      label: item.name,
                      value: item.slug,
                    ...
                    {router.query.skills?.split(",").map((item, i) => (
                      <div key={i} className="flex items-center gap-0.5">
                        ...
                        <div
                          onClick={() =>
                            router.push(
                              removeQueryValue(router, "skills", item),
                            )
                          }
                          className="p-2 px-4 hover:bg-zinc-500/10 bg-zinc-500/5 rounded-r-lg transition-all duration-200 cursor-pointer"
                        >
          ...
          <InfiniteScrollComponent
            ...
            preRenderCount={9}
            preRender={(item, i) => <MiniCard key={i} isSkeleton />}
            itemsCount={12}
            render={(item, i) => (
              <MiniCard
                key={i}
