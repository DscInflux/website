...
import "next-bottom-sheet/lib/min.css";
...
import seoConfig from "../configurations/seo.config";
        ...
        await axios
          ...
          .then((res) => {
            ...
            if (data.status === "ok") {
              ...
              if (hasWindow) {
                if (window.opener) {
                  window.location.href = "/v1/auth/login?next=" + next || "/";
                } else {
                  window.location.href = "/v1/auth/login?next=" + next || "/";
                }
              } else {
                router.push("/v1/auth/login?next=" + next || "/");
              }
            }
          });
      }
  ...
  return (
    <>
      ...
      {router.pathname && (
        <NextSeo
          title={
            replaceValues(influxConfig.titles?.[router.pathname]?.title) ||
            influxConfig.titles?.["/"].title
          }
        />
      )}
      <Toaster
