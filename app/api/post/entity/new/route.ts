...
export async function POST(req: NextRequest) {
  ...
  const existingUrl = await prisma.entity.findFirst({ where: { url: data.url, deletedAt: null } });
  if (existingUrl && existingUrl.discordId !== session.user.id) {
    return NextResponse.json({ error: 'URL already taken' }, { status: 400 });
  }

  const existingEntity = await prisma.entity.findFirst({ where: { discordId: session.user.id, deletedAt: null } });
  const now = new Date();
