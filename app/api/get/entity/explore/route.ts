...
export async function GET(req: NextRequest) {
  ...
  const {
    page, limit, sort, roles, skills, isVerified, isPremium, isPartner, isDeveloper, staff,
    search, location, gender, language
  } = parseResult.data;
  ...
  if (sort === 'random') {
    const allIds = await prisma.entity.findMany({ select: { id: true }, where: filters });
    const shuffled = allIds.map(e => e.id).sort(() => Math.random() - 0.5).slice(0, take);
    const entities = await prisma.entity.findMany({ where: { id: { in: shuffled } } });
