...
export function generateExploreMetadata(params: MainMetaDataParam): Metadata {
	return generateMetadata({
		...
		image: params.image ?? defaultImage,
		keywords: params.keywords?.length ? params.keywords : ['Explore', 'Sociava'],
		canonicalUrl: params.canonicalUrl
	});
}
...
export function generateSigninMetadata(params: MainMetaDataParam): Metadata {
	return generateMetadata({
		...
		image: params.image ?? defaultImage,
		keywords: params.keywords?.length ? params.keywords : ['Sign In', 'Sociava'],
		canonicalUrl: params.canonicalUrl
	});
}
...
export function generateUserSettingsMetadata(params: MainMetaDataParam): Metadata {
	return generateMetadata({
		...
		image: params.image ?? defaultImage,
		keywords: params.keywords?.length ? params.keywords : ['User Settings', 'Sociava'],
		canonicalUrl: params.canonicalUrl
	});
}

export function generateUserMetadata(params: UserMetaDataParam): Metadata {
	const { name, profilePicture, banner, biography, keywords, canonicalUrl } = params;
